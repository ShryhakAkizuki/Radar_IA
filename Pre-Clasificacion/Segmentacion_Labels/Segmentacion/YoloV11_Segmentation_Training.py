...
def copiar(path :str, imagenes :list, subset: str) -> None:
    ...
    for i, (ruta, segmentos) in enumerate(imagenes):                # Analiza todos los elementos de la lista de imagenes, en un formato de dupla doble (i, (ruta, segmento)).
        label = "1" if segmentos!="N/A" else "0"                    # Si el segmento es "N/A" (no tiene) su respectivo label es 0 (no hay embarcacion), de otra manera es 1 (Hay embarcacion)
...
def YOLO_Training(path: str, results_name: str, nombre_yaml: str) -> None:
    ...
    model.train(
        ...
        imgsz=640,                                           # Tamaño de imagen recomendado para cls
        batch=32,                                            # Tamaño del batch
        project=f"{path}\\runs",                             # Carpeta donde guarda resultados
        name=results_name,                                   # Nombre del experimento
        device = 'cpu',                                      # Dispositivo que se encargara del entrenamiento
        workers=16,                                          # Numero de Hilos
    )
...
if __name__ == "__main__":
    ...
    # -------- Variables ------------------------------------------------------------
    csv_path    = "..\\Registros_Segmentados.csv"                   # Ruta de la carpeta donde se encuentra el archivo CSV
    output_dir  = "..\\Model_Training\\YoloV11_Semi-Segmentation"   # Ruta donde se copiara el Dataset en el formato adecuado
    ...
    if not os.path.exists(os.path.join(output_dir,"Dataset")):                  # Si la carpeta del Dataset no existe, lo crea y organiza
        ...
        Generador_Yaml(output_dir, {0: "sin_embarcacion", 1: "embarcacion"}, "dataset.yaml")    # Genera el archivo Yaml

    else:
        print("El path ya existe por ende se procede al entrenamiento del modelo")

    # # ---------------- Realizar el entrenamiento del modelo -------------------------
    YOLO_Training(output_dir,"Modelo_Entrenado", "dataset.yaml") 