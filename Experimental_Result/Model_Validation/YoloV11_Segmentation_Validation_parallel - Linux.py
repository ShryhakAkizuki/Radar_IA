...
if __name__ == "__main__":
    ...
    num_workers  = math.floor(5)                                                                                            # Cantidad de procesos que se crean
    ...
    output_path  = f"../YoloV11_Segmentation-Results_RTXA4000_Batch_{batch_size}_parallelV2_{num_workers}_workers.csv"      # Ruta donde se exportaran los resultados
    ...
    with open(csv_path, mode="r", newline='', encoding='utf-8') as infile:       
        ...
        for row in reader:                                                              # Por cada elemento del archivo CSV
            ...
            if row['Image_Name']!="N/A":                                                # Si el elemento contiene una imagen la procesa
                Ruta_Image = row["Path"].replace("\\", "/")
                Ruta_Image = os.path.normpath(os.path.join(                             # Arma la ruta de la imagen
                    Ruta_Image, 
                    row["Image_Path"], 
                    row["Image_Name"]))    # Arma la ruta de la imagen
